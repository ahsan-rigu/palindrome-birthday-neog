...
function creatAllDateFormats(date) {
  ...
  var ddmmyyyy =
    dateStringObject.day + dateStringObject.month + dateStringObject.year;
  var mmddyyyy =
    dateStringObject.month + dateStringObject.day + dateStringObject.year;
  var yyyymmdd =
    dateStringObject.year + dateStringObject.month + dateStringObject.day;
  var ddmmyy =
    dateStringObject.day +
    dateStringObject.month +
    dateStringObject.year.slice(-2);
  var mmddyy =
    dateStringObject.month +
    dateStringObject.day +
    dateStringObject.year.slice(-2);
...
function clickHandeller() {
  ...
  if (dateString !== "") {
    ...
    if (checkPallindromeForAllDates(date)) {
      outputDivRef.textContent = "Yay! your birthday is a pallindrome";
    } else {
      var [daysFromPallindrome, closestDate] = findClosestPallindromeDate(date);
      outputDivRef.textContent =
        "the next pallindrome date is " +
        closestDate.day +
        "-" +
        closestDate.month +
        "-" +
